...
// You can execute this example with `go test serialize_test.go`
...
func serialize() []byte {
	...
	module, err := wasmtime.NewModuleFromFile(engine, "hello.wat")
	if err != nil {
		panic(err)
	}
...
func deserialize(encoded []byte) {
	...
	hello_func := wasmtime.WrapFunc(store, func() {
	...
	instance, err := wasmtime.NewInstance(store, module, []*wasmtime.Extern{hello_func.AsExtern()})
	if err != nil {
		panic(err)
	}
